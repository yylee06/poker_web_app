...
function Actions({ socket }) {
    ...
    const callbackCurrentChips = useCallback(() => {
        ...
        function getCurrentChips() {
            fetch('https://yunyeollee-server.com:3080/current_chips', {method: 'POST', body: JSON.stringify({token: token_parsed?.token}), headers: player_headers})
            .then(res => res.json())
    ...
    useEffect(() => {
        fetch('https://yunyeollee-server.com:3080/highest_bet')
            .then(res => res.json())
            .then((retrievedMessage) => {
    ...
    function onSubmit() {
        ...
        fetch('https://yunyeollee-server.com:3080/raise', {method: 'POST', body: raise_body, headers: player_headers})
            .then(res => res.json())
            .then((retrievedMessage) => {
                if (retrievedMessage.auth === 1) {
    ...
    function doAction(action) {
        ...
        switch(action) {
            case "check":
                fetch('https://yunyeollee-server.com:3080/check', {method: 'POST', body: JSON.stringify({token: token_parsed?.token}), headers: player_headers})
                    .then(res => res.json())
                    .then((retrievedMessage) => {
                        if (retrievedMessage.auth === 1) {
                            console.log(retrievedMessage.message)
                ...
                break;
            case "call":
                fetch('https://yunyeollee-server.com:3080/call', {method: 'POST', body: JSON.stringify({token: token_parsed?.token}), headers: player_headers})
                    .then(res => res.json())
                    .then((retrievedMessage) => {
                        if (retrievedMessage.auth === 1) {
                            console.log(retrievedMessage.message)
                ...
                break;
            case "fold":
                fetch('https://yunyeollee-server.com:3080/fold', {method: 'POST', body: JSON.stringify({token: token_parsed?.token}), headers: player_headers})
                    .then(res => res.json())
                    .then((retrievedMessage) => {
                        if (retrievedMessage.auth === 1) {
                            console.log(retrievedMessage.message)
