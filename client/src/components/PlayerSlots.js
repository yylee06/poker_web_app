...
function PlayerSlots({ socket, setIngameToken }) {
    ...
    const callbackAchievements = useCallback(() => {
      function getAchievementsState() {
        fetch('https://yunyeollee-server.com:3080/achievements')
          .then(res => res.json())
    ...
    const callbackPlayerChipsState = useCallback(() => {
      function getPlayerChipsState() {
        fetch('https://yunyeollee-server.com:3080/player_chips')
          .then(res => res.json())
          .then((retrievedMessage) => {
    ...
    const callbackPlayerState = useCallback(() => {
        ...
        function getPlayerState() {
          fetch('https://yunyeollee-server.com:3080/players', {method: 'POST', body: JSON.stringify({token: token_parsed?.token}), headers: player_headers})
          .then(res => res.json())
          .then((retrievedMessage) => {
            let current_players = [];
    ...
    const callbackIngameToken = useCallback((isValid) => {
        ...
        function getIngameToken() {
          fetch('https://yunyeollee-server.com:3080/ingame_token', {method: 'POST', body: JSON.stringify({token: token_parsed?.token}), headers: player_headers})
          .then(res => res.json())
          .then((retrievedMessage) => {
