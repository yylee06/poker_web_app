...
function StartGame() {
    ...
    function toggleGame(toggled_value) {
        fetch('http://localhost:3080/toggle_game', {method: 'POST', body: JSON.stringify({token: token_parsed?.token, toggle: toggled_value, begin_game: 1}), headers: player_headers})
        .then(res => res.json())
