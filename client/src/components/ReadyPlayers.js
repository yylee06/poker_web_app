...
function ReadyPlayers({ socket, setGameToken, ingameToken }) {
    ...
    const callbackReadyPlayersState = useCallback(() => {
        function getReadyPlayersState() {
          fetch('https://yunyeollee-server.com:3080/ready_players')
            .then(res => res.json())
    ...
    const callbackPlayerStillInGame = useCallback(() => {
        function getPlayerInGameState() {
            ...
            fetch('https://yunyeollee-server.com:3080/player_still_ingame', {method: 'POST', body: JSON.stringify({token: token_parsed?.token}), headers: player_headers})
                .then(res => res.json())
                .then((retrievedMessage) => {
