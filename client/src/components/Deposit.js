...
function Deposit() {
  ...
  useEffect(() => {
    ...
    fetch('https://54.91.205.171:3080/show_chips_useable', {method: 'POST', body: JSON.stringify({token: token_parsed?.token}), headers: deposit_headers})
    .then(res => res.json())
  ...
  function onSubmit() {
    ...
    fetch('https://54.91.205.171:3080/deposit', {method: 'POST', body: deposit_body, headers: deposit_headers})
      .then(res => res.json())
      .then((retrievedMessage) => {
        if (retrievedMessage.auth === 1) {
          setChips(retrievedMessage.amount)
