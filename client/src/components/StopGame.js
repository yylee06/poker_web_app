...
function StopGame() {
    ...
    function toggleGame(toggled_value) {
        fetch('https://54.91.205.171:3080/toggle_game', {method: 'POST', body: JSON.stringify({token: token_parsed?.token, toggle: toggled_value, begin_game: 0}), headers: player_headers})
        .then(res => res.json())
