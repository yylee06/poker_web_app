...
function Withdraw() {
  ...
  React.useEffect(() => {
    ...
    fetch('https://yunyeollee-server.com:3080/show_chips_bank', {method: 'POST', body: JSON.stringify({token: token_parsed?.token}), headers: withdraw_headers})
    .then(res => res.json())
  ...
  function onSubmit() {
    ...
    fetch('https://yunyeollee-server.com:3080/withdraw', {method: 'POST', body: withdraw_body, headers: withdraw_headers})
      .then(res => res.json())
      .then((retrievedMessage) => {
        if (retrievedMessage.auth === 1) {
          setChips(retrievedMessage.amount)
